...
time_series <- create_deseq("time_series", time_series_txi, ~Pesticide + Time)
time_2h <- results(time_series, contrast(""))

#+end_src

#+RESULTS:
: /tmp/babel-e2VlnV/R-duXQpC

**** Functions for selecting genes
These are utility functions for selecting the genes based on =pvalue= and =log2FoldChange=
#+begin_src R :tangle scripts/analysis/deseq2.r :session deseq :async
...
main <- list(deet, perm, body_leg)
# apply function to every member of data
# select for p value less than 0.01
main <- sapply(data, significant, threshold=0.01)
# select for genes with log2FoldChange larger than 2
up_regulated = sapply(main, upRegulated, threshold=2)
# select for genes with log2FoldChange less than -2
down_regulated = sapply(main, downRegulated, threshold=-2)
#+end_src
